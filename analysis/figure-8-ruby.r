...
dat_glibc <- read.delim('results/3-ruby/memory/libc.tsv', header = TRUE, sep = '\t')
...
dat_jemalloc <- read.delim('results/3-ruby/memory/jemalloc.tsv', header = TRUE, sep = '\t')
...
dat_mesh2n <- read.delim('results/3-ruby/memory/mesh2n.tsv', header = TRUE, sep = '\t')
dat_mesh2n$alloc <- 'Mesh (no meshing)'
dat_mesh2n$heap <- (dat_mesh2n$rss + dat_mesh2n$kernel) / 1024.0 / 1024.0
dat_mesh2n$time <- dat_mesh2n$time / 1000000000.0

dat_mesh0y <- read.delim('results/3-ruby/memory/mesh0y.tsv', header = TRUE, sep = '\t')
dat_mesh0y$alloc <- 'Mesh (no rand)'
dat_mesh0y$heap <- (dat_mesh0y$rss + dat_mesh0y$kernel) / 1024.0 / 1024.0
dat_mesh0y$time <- dat_mesh0y$time / 1000000000.0

dat_mesh2y <- read.delim('results/3-ruby/memory/mesh2y.tsv', header = TRUE, sep = '\t')
...
p <- ggplot() +
    geom_line(data=dat_mesh2n, size=.3, aes(x=time, y=heap, color=alloc, group=2)) +
    geom_line(data=dat_mesh0y, size=.3, aes(x=time, y=heap, color=alloc, group=3)) +
...
ggsave(p, filename='results/3-ruby/figure-8-ruby.pdf', width=3.4, height=1.5)
