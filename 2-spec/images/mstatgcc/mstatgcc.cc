...
#define LAIR_DIR "/spec"
#define EVIL_LAIR LAIR_DIR "/native-binaries"
#define DATA_LAIR LAIR_DIR "/result/mstat"
...
int main(int argc, char **argv) {
  ...
  // we will build a command into cmd that invokes the real compiler with the
  // args we were passed

  std::string cmd;

  mkdir(LAIR_DIR, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
  ...
  for (int i = 1; i < argc; i++) {
    ...
    // see if we are making an executable (i.e. using -o and not making an
    // object file)

    if (!strcmp(argv[i], "-o") &&
        strcmp(&argv[i + 1][strlen(argv[i + 1]) - 2], ".o")) {
      char *binary = argv[i + 1];

      // ah-hah! we apparently made an executable. let's kidnap it to our evil
      // lair and leave in its place a doppelganger
      ...
      // make a shell script that does perf or whatever and invokes the real
      // executable

      FILE *f = fopen(binary, "w");
      fprintf(f,
              "#!/bin/bash\nDATE=`date +%%s.%%N`\nSUFFIX=`cat /dev/urandom | "
              "tr -cd 'a-f0-9' | head -c 8`\nexec %s -o "
              "%s/%s-$DATE-$SUFFIX.tsv -- %s \"$@\"\n",
